...
- **Ixly-afronding: nieuwe schrijfbare WooCommerce-sleutel aanmaken** `(lokaal)` — de fix voor de kapotte Ixly-statuscontrole (zie design-doc van 2026-08-01: docs/superpowers/specs/2026-08-01-ixly-assignment-uuid-persisted-design.md) heeft een NIEUWE, schrijfbare WooCommerce REST-sleutel nodig (los van de bestaande alleen-lezen sleutel van Apps Script). Aanmaken via WooCommerce → Instellingen → Geavanceerd → REST API (permissies: lezen/schrijven), en als GitHub Secrets zetten (niet als Azure Portal App Setting -- zie ADR-003): `GROVIA_WORDPRESS_URL`, `GROVIA_WOO_CONSUMER_KEY`, `GROVIA_WOO_CONSUMER_SECRET`. Zonder deze secrets wordt `_grovia_ixly_taken` niet bewaard (gelogd, geen fout) en blijft een nieuwe order net als de bestaande ~31 rijen op handmatige Ixly-controle staan. Controleer daarom na de testorder hieronder ook in de Azure-logs dat er GEEN "niet (volledig) gezet"-regel of "Kon _grovia_ixly_taken niet bewaren"-regel voorkomt (het faalpad is bewust stil, dus dit is de enige manier om een verkeerd gezette secret te ontdekken).
